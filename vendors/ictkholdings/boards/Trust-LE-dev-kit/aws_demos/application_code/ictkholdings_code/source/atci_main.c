...
#ifdef MTK_MUX_ENABLE
atci_status_t atci_read_mux_data(uint32_t data_len);
#else
...
#ifdef MTK_MUX_ENABLE
extern mux_handle_t atci_mux_handle;
extern mux_port_t g_atci_port;
...
#ifdef MTK_MUX_ENABLE
    status = mux_open(g_atci_port, "ATCI", &atci_mux_handle, mux_atci_callback, NULL);
    if (status != MUX_STATUS_OK) {
        assert(0);
    }

    if (status == MUX_STATUS_OK) {
        LOGI("%s mux_open success \r\n",ATCI_INTT_INT);
        atci_init_int();
    } else {
        assert(0);
    }
...
#ifdef MTK_MUX_ENABLE
{
    ...
    pdata.p_buf = buf;
    if(MUX_STATUS_OK != mux_tx(atci_mux_handle, &pdata,1, &data_len)) {
         assert(0);
    }
    return data_len;
}
...
uint32_t atci_port_read_data(uint32_t port, uint8_t *buf, uint32_t buf_len)
{
    ...
    if (MUX_STATUS_OK == mux_rx(atci_mux_handle, &buffer, &data_len)) {
         //return data_len;
    } else {
        // should read one package at a time, otherwise it returns MUX_STATUS_USER_RX_BUF_SIZE_NOT_ENOUGH 
        assert(0);
    }
...
atci_status_t atci_read_mux_data(uint32_t data_len)
{
    ...
    uint32_t  input_data_len = data_len;
    uint32_t  read_data_len = 0;

    LOGI("%s, enter,g_atci_port_handle:%x, input_data_len = %d\r\n", ATCI_READ_DATA, g_atci_port_handle,input_data_len);

    if (input_data_len >= ATCI_UART_RX_FIFO_BUFFER_SIZE) {
        assert(0);
    }
...
void atci_processing(void)
{
    ...
    while (1) {
        ...
        if (common_queue_msg_num > 0) {
            ...
            LOGI("%s msg_data(%x)\\r\n", ATCI_PROCESSING, data_len);
            atci_read_mux_data(data_len);
            #else 
                atci_read_data();
            #endif
...
void mux_atci_callback(mux_handle_t handle, mux_event_t event,uint32_t data_len,void*user_data)
{
    ...
    switch (event) {
        ...
        {
            if (atci_check_startup_finish()) {
                LOGI("%s 1, read event, data_len %d \r\n",ATCI_SERIAL_PORT_DATA_CALLBACK, data_len);
                msg_queue_item.msg_id = MSG_ID_ATCI_LOCAL_SEND_CMD_IND;
                g_data_len = data_len;
                msg_queue_item.msg_data = &g_data_len;
                atci_queue_send(g_atci_input_command_queue, (void *)&msg_queue_item);
                atci_semaphore_give(g_atci_handler_semaphore);
            }
            break;
