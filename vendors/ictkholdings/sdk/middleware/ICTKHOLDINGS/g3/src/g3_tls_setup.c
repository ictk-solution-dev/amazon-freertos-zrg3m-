...
static int tls_setup()
{
	...
	uint8_t certInfo[4] = {0,};
        ...
        //--- certificate setup-------------------------------------------------------------------------
        char key_pem[G3_MAX_CERT_SIZE];
        char cert_pem[G3_MAX_CERT_SIZE];
        
        if( strcmp(tlstestCLIENT_CERTIFICATE_PEM_EC, "Paste client certificate here.") == 0)
        {
            strcpy(key_pem, keyCLIENT_PRIVATE_KEY_PEM);
            strcpy(cert_pem, keyCLIENT_CERTIFICATE_PEM);
        }
        else
        {
            strcpy(key_pem, tlstestCLIENT_PRIVATE_KEY_PEM_EC);
            strcpy(cert_pem, tlstestCLIENT_CERTIFICATE_PEM_EC);
        }
        
        //static const char *base64_client_prvkey = (const char*) key_pem;
...
#error   "G3_CLIENT_CERT_PRIVKEY_KEYSECTOR" must be 114
        ...
        base64_address = NULL;
        char *key_data_address = NULL;
        key_data_address = strstr(key_pem, G3_PEM_EC_HEADER);
        if(key_data_address == NULL)
        {
            base64_address = delete_header_footer((unsigned char*)key_pem, &prvlen_base64);
        }
        else
        {
            base64_address = delete_header_footer((unsigned char*)key_data_address, &prvlen_base64);
        }
	...
	ret = g3api_write_key_value(G3_CLIENT_CERT_PRIVKEY_KEYSECTOR, KEY_AREA, PLAIN_TEXT, &tx_buffer[0], size);
        ...
        for(int i =0; i< (((cert_size-1)/32)+1); i++)
        {
            ...
            memcpy(tx_buffer, (void *) (cert_pem+32*i), size);
