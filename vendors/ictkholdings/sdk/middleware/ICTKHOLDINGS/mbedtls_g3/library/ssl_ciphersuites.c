...
static const int ciphersuite_preference_RSA[] =
{
  MBEDTLS_TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256//MBEDTLS_SSL_CIPHERSUITES_RSA
};
...
{
  if(cipher_flag == 0)
    return( ciphersuite_preference_default );
  else
    return( ciphersuite_preference_RSA );
}
#else
...
const int *mbedtls_ssl_list_ciphersuites( void )
{
    ...
    if( supported_init == 0 )
    {
      ...
      if(cipher_flag==0)
      {
        for( p = ciphersuite_preference_default, q = supported_ciphersuites;
      ...
      {
        for( p = ciphersuite_preference_RSA, q = supported_ciphersuites;
             *p != 0 && q < supported_ciphersuites + MAX_CIPHERSUITES - 1;
             p++ )
        {
#if defined(MBEDTLS_REMOVE_ARC4_CIPHERSUITES)
