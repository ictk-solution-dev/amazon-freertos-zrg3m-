...
int ictktls_ecdsa_ota_verify_signature( const unsigned char *sig, size_t slen )
{
    int ret = 0;
    unsigned char *p = (unsigned char *) sig;
      const unsigned char *end = sig + slen;
...
//    memcpy(&(sign.s[0]),&s_buf[0] ,32);
    ...
    if(ret != 0)
    {
     // IOT_INFO("[ICTK]signature verification : failure\r\n");
    }
    else
    {
      memcpy(&u8Signature[0], &r_buf[0], 32);
      memcpy(&u8Signature[32], &s_buf[0], 32);
     // IOT_INFO("[ICTK]signature verification : success\r\n")
    }
