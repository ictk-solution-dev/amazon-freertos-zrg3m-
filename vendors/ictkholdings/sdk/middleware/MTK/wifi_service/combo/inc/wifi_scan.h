...
#define WPA_WIFI_API_MUTEX_TAKE_FAIL    ("api mutex take fail")
...
#define PACKED ATTR_PACKED
 ...
 /**
 * @brief The inferface is used to get/set/query/operation minisupp used in wifi_service.
 * It should implement in minisupp.
 * In our desgin, there are 3 class of functions
 * 1. get config and status, direct access
 * 2. set config and call standalone function(not effect the SM state), sock send
 * 3. operation/Action , sock send
 * Because of item2 and item3 maybe have time issue, it not allowed t0 directly invoked by wifi_service, replace it with sock to minisupp task.
 * Refer the wifi_event_id.h define. Some operation need synchronzie, or some should be asynchronous
**/
 typedef struct __wpa_supplicant_api_callback {
    ...
    int (*wps_get_pin)(uint8_t port, uint8_t *pin_code);
    int (*wps_get_auto_connection)(bool *auto_connection);
    int (*wps_connect_pbc)(uint8_t port, uint8_t *bssid);
    int (*wps_connect_pin)(uint8_t port, uint8_t *bssid, uint8_t *pin_code);
    int (*wps_get_device_info)(wifi_wps_device_info_t *device_info);
#endif
